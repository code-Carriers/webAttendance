...
<SCRIPT type="text/javascript">
	...
	function banzujiazai() {
		...
		for ( var i = 0; i < data.length; i++) {
			...
			var xinxi;
			if (renshu > 0) {
				var a = "<a style='margin-right:13px;'  href=javascript:paiban("+data[i].banzuid+")>安排班次</a>";
				...
				xinxi = "<td><a style='margin-right:13px;'  href=javascript:anPaiBanCi("+data[i].banzuid+")>查看排班信息</a></td>";
			} else {
...
<!-- 新加 -->
	<div style="width:inherit;left: 42%;top: 25%;"
		class="modal hide fade in" id="paiban" tabindex="-1"
		role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div style="text-align: center;">
						<div>
							<table>
								<tr style="height: 50px;">
									...
									<td><input style="width: 150px;" name="startdate" id="startDate" type="text">
								 	 <script type="text/javascript">
								$("#startDate").jeDate({
							    isinitVal:false,
							    ishmsVal:false,
							    ...
							    zIndex:3000,
									...
									<td ><input name="enddate" style="width: 150px;" id="endDate" type="text"> 
									 <script type="text/javascript">
								$("#endDate").jeDate({
							    isinitVal:false,
							    ishmsVal:false,
								...
								<tr style="height: 70px;">
									...
									<input type="checkbox" name="xingqi" value="1">星期一
									<input type="checkbox" name="xingqi" value="2">星期二
									<input type="checkbox" name="xingqi" value="3">星期三
									<input type="checkbox" name="xingqi" value="4">星期四
									<input type="checkbox" name="xingqi" value="5">星期五
									<input type="checkbox" name="xingqi" value="6">星期六
									<input type="checkbox" name="xingqi" value="0">星期天
									</td>
								</tr>
								<tr>
									<td>班次：</td>
									...
									<select id="banci">
										<option id="xzBanci" value="">选择班次</option>
									</select>
						...
						<div class="form-actions center">
							<button type="button" onclick="riqipaiban()"
								class="btn btn-sm btn-success">
								保 存 <i class="icon-check icon-on-right bigger-110"></i>
							</button>
							<button type="reset" class="btn btn-sm btn-warning"
...
<script type="text/javascript">
  ...
  /**点击安排班次时触发 **/
  function paiban(banzuid) {
	 dangQianBanZuId=banzuid;
   	 var BanCiData = ajax("${ctx}/Scheduling/selectClassTimes.do?groupid="+banzuid);
	 $("#banci").empty();
	 for ( var i = 0; i < BanCiData.length; i++) {
			var html = "<option style='text-align: center;' value='"
					+ BanCiData[i].positionclassesid
					+ "'>"
					+ BanCiData[i].positionclasses.classesname + "</option>";
			//alert( BanCiData[i].positionclassesid);		
		 	$("#banci").append(html);
	  }
	  $("#paiban").modal("show");
  }
 ...
 function riqipaiban(){
	...
	var start_date= $("#startDate").val();
 	...
 	var obj = document.getElementsByName("xingqi");
 	check_val = [];
    for(var k in obj){
        if(obj[k].checked)
    ...
    if(check_val.length == 0){
 	...
 	if(isDate(start_date)&&isDate(end_date)){
  		if(panduanshijian(start_date,end_date)){
	   	 	var options=$("#banci option:selected");  //获取选中的项 
	   	 	if(options.val() ==""){
	   	 		alert("请选择班次");
	   	 		return false;
	   	 	}
			var shulian= ajax("${ctx}/Scheduling/paiban.do?"+
			 "xingqi="+check_val+"&startDate="+start_date+
			 "&endDate="+end_date+"&classid="+options.val()+
			 "&groupid="+dangQianBanZuId); 
     		if(shulian == "-1"){
   				alert("保存失败");
   	 		}
    	 	else if(shulian>-1){
  ...
  function panduanshijian(beginDate,endDate){
   ...
   for(var i=1;i<tianshu+1;i++){
	...
	/* 
	headertr=headertr+"<div style='padding-left: 10px;'>"
			+"<table border='1' class='table' style='max-height:70%;min-height:50%; max-width: 69%;min-width: 69%; ' id='tbGudinglunban'>"
				+"<thead id='header'>"
					+"<tr>"
						+"<th>星期一</th>"
						+"<th>星期二</th>"
				...
				+"<tbody id='tbodyGeRenPaiBanXinXi' class='tbody'>"		
				  ...
				  for(var i=1;i<tianshu+1;i++){
					 	var ri=(i<10?"0"+i:i);
	   					var yueRi=(parseInt(yuefen)<10?"0"+yuefen:yuefen)+"-"+ri;
	   					week=new Date(nianfen+"-"+yueRi).getDay();
	   			
						if(i==1){
							
							if(week==0)
								for(var k=0;k<6;k++)
									 td=td+"<td style='height:50px;'></td>";
							else
								for(var j=1;j<week;j++)
								 td=td+"<td style='height:50px;'></td>";
							 td=td+"<td style='height:50px;' onclick='gaibiantdyanshi(this)'><p style='color:blue;font-size:20px;'>"+i+"</p></td>";	
						}
						else  td=td+"<td style='height:50px;' onclick='gaibiantdyanshi(this)'><p style='color:blue;font-size:20px;'>"+i+"</p></td>";	
						if(week==0&&i!=tianshu)
							td=td+"</tr><tr>";
				
				}		 
				headertr=headertr+td+"</tr></tbody></table></div>"; 
   ...
   return true;  
	...
	if(before.length>0){
	 if(before[0].children[0].innerHTML!=classid){
	  ...
	  var html= "<div name='Insert'><span name='classid' style='display:none'>"
	  +classid+"</span><span name='workdate' style='display:none'>"
	  +workdate+"</span><span name='staffid' style='display:none'>"
	  +this.parentNode.id.substr(4)+"</span>"	  
	  +classname+"</div>";
	  $(this).append(html);
	}
	});
